...
In a single VNet, private endpoint DNS is straightforward: create the private DNS zone, link it to the VNet, and name resolution works. In a hub-and-spoke topology it's a different story. DNS queries from spokes are resolved by something in the hub - a custom DNS server or an Azure DNS Private Resolver - and if your private DNS zones aren't visible from there, you silently get the public IP back. Every time.

This post covers the three common resolution patterns and where each one breaks.

---

## Why the single-VNet approach breaks in hub-and-spoke

In hub-and-spoke, spoke VNets don't use Azure's default DNS (`168.63.129.16`) directly. Their DNS server setting points at a forwarder in the hub - a custom DNS VM or a DNS Private Resolver inbound endpoint. All queries go there first.

Azure resolves private DNS zones only for VNets to which the zone is **linked**. If you link `privatelink.blob.core.windows.net` to a spoke VNet but DNS queries are actually resolved by something in the hub, the zone link on the spoke does nothing. The hub resolver has no knowledge of the zone.
...
The key requirement: the DNS server must sit in - or be peered to - a VNet that has the zones linked. A custom DNS VM in the hub that isn't linked to the `privatelink.*` zones will still return public IPs, regardless of how well the forwarder is configured.
...
If you have separate hub VNets per region (`hub-uksouth`, `hub-uknorth`), each hub needs its own DNS configuration and its own set of zone links. A private DNS zone linked to `hub-uksouth` is not visible to a resolver running in `hub-uknorth`.
...
DNS Private Resolver inbound endpoints require an empty, dedicated subnet. Applying NSGs with restrictive inbound rules to this subnet - particularly blocking UDP 53 from spoke address spaces - will silently drop DNS queries. Use a dedicated subnet with an NSG that explicitly permits DNS traffic from your spoke ranges.
