...
Think of a VNet as the Azure equivalent of a traditional on-premises LAN or VLAN, except it's software-defined, spans availability zones automatically, and the best part, you never have to rack a switch.

Within Azure there is a need for network isolation, without it, Azure resources would sit in a flat, shared network. VNets give you logical isolation. Your resources are segregated from other Azure customers and from other VNets (unless peered), with full control over IP addressing, routing, and access.
...
- **DDoS protection** — Infrastructure protection (formerly basic) is free. Standard is a paid per-plan charge and covers all resources in the VNet.
...
- Azure-reserved ranges (`169.254.0.0/16`, `168.63.129.16/32`)
...
**5. Reserved subnet names have exact sizing requirements** - Several subnet names are reserved by Azure and must be named exactly (case-sensitive). Each has a minimum size, deploying undersized will cause service deployment to fail.
